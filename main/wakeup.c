...
#define WAKEUP_MASK 10
const char RTC_RODATA_ATTR time_fmt_str[] = "Time: %llu\n";
const char RTC_RODATA_ATTR bool_status[] = "Bool: %d\n";
const char RTC_RODATA_ATTR randomString[] = "Random String:\n";
...
RTC_IRAM_ATTR void deepsleep_for_us(uint64_t duration_us)
{
    ...
    REG_SET_FIELD(RTC_CNTL_WAKEUP_STATE_REG, RTC_CNTL_WAKEUP_ENA, WAKEUP_MASK); // Wake up on timer and wake pins
...
RTC_NOINIT_ATTR static time_t timeWhenToSleep;
RTC_NOINIT_ATTR static int cntThreshold;

void RTC_IRAM_ATTR wake_stub()
{
    ets_printf(randomString);
    ets_printf(bool_status, thisIsTheStart);
    REG_WRITE(TIMG_WDTFEED_REG(0), 1);
    uint64_t now = my_rtc_time_get_us();
    ...
    ets_printf(time_fmt_str, timeAtStart);
    // REG_WRITE(TIMG_WDTFEED_REG(0), 1);
    ...
    if (pinMaskS & (uint32_t)1 << interruptPinInRTC)
    {
        if ((now - lastDebounceTimeInput1) > 50000)
        {
            ets_printf(innerFmtStr);
            event_t thisEvent;
            thisEvent.timeStamp = now;
            thisEvent.outerBarrier = false;
            events[eventCount] = thisEvent;
            eventCount++;
        }
        lastDebounceTimeInput1 = now;
    }
    REG_WRITE(TIMG_WDTFEED_REG(0), 1);

    if (pinMaskS & (uint32_t)1 << interruptPinOutRTC)
    {
        if ((now - lastDebounceTimeInput2) > 50000)
        {
            ets_printf(outerFmtStr);
            event_t thisEvent;
            thisEvent.timeStamp = now;
            thisEvent.outerBarrier = true;
            events[eventCount] = thisEvent;
            eventCount++;
        }
        lastDebounceTimeInput2 = now;
    }
    REG_WRITE(TIMG_WDTFEED_REG(0), 1);
